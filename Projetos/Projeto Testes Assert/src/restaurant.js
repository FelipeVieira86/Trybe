...
const orderFromMenu = (request) => {
  if (
    restaurant.fetchMenu.food[request] ||
    restaurant.fetchMenu.drinks[request]
  ) restaurant.consumption.push(request);
  else console.log(`${request} não encontrado no cardapio`);
};
...
const createMenu = (myMenu) => {
  restaurant = {
    ...
    pay() {
      let total = 0;
      this.consumption.forEach((product) => {
        if (myMenu.food[product]) total += myMenu.food[product];
        else total += myMenu.drinks[product];
      });
...
console.log(
  `${meuRestaurante.consumption.map(
    product =>
      ` ${meuRestaurante.consumption.find(
        product2 => product === product2
      )}: ${
        meuRestaurante.fetchMenu.food[product]
          ? meuRestaurante.fetchMenu.food[product].toFixed(2)
          : meuRestaurante.fetchMenu.drinks[product].toFixed(2)
      }`
  )} + 10% = ${meuRestaurante.pay()}`
);
